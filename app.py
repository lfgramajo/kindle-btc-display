#!/usr/bin/env python3
"""Kindle Bitcoin Price Display — v1.7.0-rc2 public release candidate.
...
# Fetch live BTC main price, MXN price, 24h change, and coin table prices.
...
# Fallback conversion used only if an API gives USD but not MXN.
...
def fetch_btc_main_coingecko() -> dict[str, Any] | None:
    try:
        data = request_json(
            f"{COINGECKO_BASE}/simple/price",
            params={"ids": "bitcoin", "vs_currencies": "usd,mxn", "include_24hr_change": "true"},
            timeout=10,
        )
        btc = data.get("bitcoin", {})
        price_usd = numeric_price(btc.get("usd"))
        price_mxn = numeric_price(btc.get("mxn"))
        ...
        return {"price_usd": price_usd, "price_mxn": price_mxn, "change_pct": change_pct, "change_abs": change_abs, "provider": "coingecko"}
    except Exception as e:
...
def fetch_btc_main_cryptocompare(config: dict[str, Any]) -> dict[str, Any] | None:
    try:
        data = request_json(f"{CRYPTOCOMPARE_BASE}/pricemultifull", params={"fsyms": "BTC", "tsyms": "USD,MXN"}, timeout=10)
        raw = data.get("RAW", {}).get("BTC", {})
        usd = raw.get("USD", {})
        mxn = raw.get("MXN", {})
        price_usd = numeric_price(usd.get("PRICE"))
        price_mxn = numeric_price(mxn.get("PRICE")) or numeric_price(float(price_usd) * float(config["usd_mxn_fallback_rate"]) if price_usd else None)
        ...
        if price_usd is None:
            return None
        return {"price_usd": price_usd, "price_mxn": price_mxn, "change_pct": change_pct, "change_abs": change_abs, "provider": "cryptocompare"}
    except Exception as e:
...
def fetch_btc_main_coinpaprika(config: dict[str, Any]) -> dict[str, Any] | None:
    try:
        ...
        price_mxn = numeric_price(float(price_usd) * float(config["usd_mxn_fallback_rate"]) if price_usd else None)
        change_abs = int(round(float(price_usd) * (float(change_pct) / 100))) if price_usd and isinstance(change_pct, (float, int)) else "N/A"
        if price_usd is None:
            return None
        return {"price_usd": price_usd, "price_mxn": price_mxn, "change_pct": change_pct, "change_abs": change_abs, "provider": "coinpaprika"}
    except Exception as e:
...
def fetch_btc_main_binance(config: dict[str, Any]) -> dict[str, Any] | None:
    try:
        ...
        if price_usd is None:
            return None
        return {"price_usd": price_usd, "price_mxn": price_mxn, "change_pct": change_pct, "change_abs": change_abs, "provider": "binance"}
    except Exception as e:
...
def fetch_btc_main_kraken(config: dict[str, Any]) -> dict[str, Any] | None:
    try:
        ...
        price_mxn = numeric_price(price * float(config["usd_mxn_fallback_rate"]))
        if open_price > 0:
        ...
        if price_usd is None:
            return None
        return {"price_usd": price_usd, "price_mxn": price_mxn, "change_pct": change_pct, "change_abs": change_abs, "provider": "kraken"}
    except Exception as e:
...
def fetch_btc_main_live(config: dict[str, Any]) -> dict[str, Any] | None:
    providers = [
        ("coingecko", lambda: fetch_btc_main_coingecko()),
...
def render_image(
    price_usd, price_mxn, change_abs, change_pct, ohlc, ohlc_timestamps,
    ...
    mxn_str = f"{format_value(price_mxn)} MXN"
    bbox2 = draw.textbbox((0, 0), mxn_str, font=font_med)
    w2, h2 = bbox2[2] - bbox2[0], bbox2[3] - bbox2[1]
    draw.text(((WIDTH - w2) // 2, y_cursor), mxn_str, font=font_med, fill=0)
    y_cursor += h2 + 4
...
def update_once(force_network: bool = False) -> dict[str, Any]:
    with THREAD_LOCK:
        with refresh_file_lock():
            ...
            prices_stale = force_network or not cache_is_fresh(price_cache, price_ttl)
            ...
            render_image(
                main.get("price_usd", "N/A"), main.get("price_mxn", "N/A"), main.get("change_abs", "N/A"), main.get("change_pct", "N/A"),
